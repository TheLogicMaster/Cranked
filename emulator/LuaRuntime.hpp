...
inline int luaNativeWrapper(lua_State *luaContext) {
    ...
    constexpr bool takesContext = argCount != 0
                                  and (std::is_same_v<typename Helper::template NthArg<0>, Emulator *> or std::is_same_v<typename Helper::template NthArg<0>, lua_State *>);
    ...
    if constexpr (takesContext) {
        ffiArgs[0] = std::is_same_v<typename Helper::template NthArg<0>, Emulator *> ? (void *) &emulator : (void *) &luaContext;
...
inline LuaVal pushTable(Emulator *emulator) {
    lua_newtable(emulator->getLuaContext());
    return {emulator->getLuaContext(), lua_gettop(emulator->getLuaContext())};
}

inline LuaVal pushUserdataObject(Emulator *emulator, void *value, const char *metatable) {
    auto object = pushTable(emulator);
    object.setLightUserdataField("userdata", value);
    emulator->getQualifiedLuaGlobal(metatable);
 ...
 */
inline LuaVal pushOwnedUserdataObject(Emulator *emulator, void *value, const char *metatable, bool owner = true) {
    auto object = pushUserdataObject(emulator, value, metatable);
    object.setBoolField("owner", owner);
    if (owner)
        emulator->preserveLuaReference(value, object);
    return object;
}

inline bool releaseOwnedUserdataObject(Emulator *emulator, LuaVal value) {
    if (!value.getBoolField("owner"))
        return false;
    return emulator->releaseLuaReference(value.getLightUserdataField("userdata"));
}

inline LuaVal pushImage(Emulator *emulator, LCDBitmap_32 *image, bool owner) {
    return pushOwnedUserdataObject(emulator, image, "playdate.graphics.image", owner);
}

inline LuaVal pushSprite(Emulator *emulator, LCDSprite_32 *sprite, bool owner) {
...
inline LuaVal pushTime(Emulator *emulator, const std::chrono::system_clock::time_point &time) {
    LuaVal table = pushTable(emulator);
