...
// Todo: A wrapper class to hide internals
    ...
    public:
        ...
        typedef void(*InternalUpdateCallback)(Cranked *cranked);

        typedef void(*LoggingCallback)(Cranked *cranked, LogLevel level, const char *format, va_list args);
        ...
        explicit Cranked(InternalUpdateCallback updateCallback, void *userdata = nullptr);
        ...
        inline R invokeEmulatedFunction(uint32_t address, [[maybe_unused]] P... params) {
            ...
            try {
                ...
                ([&] { // C++ fold magic
                    ...
                    } else if (currentReg >= 4 or (wide and currentReg > 2) or
                               (type == ArgType::struct2_t and currentReg > 2) or
                               (type == ArgType::struct4_t and currentReg > 0)) {
                ...
                uint32_t pc = address;
                while (true) {
                    auto result = uc_emu_start(nativeEngine, pc | 0x1, returnAddress, 15000,
                                               0); // Todo: Verify/fix timing
                    pc = readRegister(UC_ARM_REG_PC);
                    if (result == UC_ERR_OK) {
                        if (pc == returnAddress)
                            break;
                        updateInternals();
                    } else if (result == UC_ERR_FETCH_UNMAPPED and pc >= FUNC_TABLE_ADDRESS and
                               pc < FUNC_TABLE_ADDRESS + FUNCTION_TABLE_SIZE * 2) {
                        uint32_t lr = readRegister(UC_ARM_REG_LR);
                        nativeFunctionDispatch(int(pc - FUNC_TABLE_ADDRESS) / 2);
                        pc = lr;
                    } else
                        assertUC(result,
                                 "Error invoking emulated function"); // Todo: Add more context for memory errors, probably need to store info from hooks, maybe full core dump
                }
    ...
    private:
        ...
        inline static int luaEmulatedFunctionDispatch(lua_State *luaContext) {
            auto cranked = fromLuaContext(luaContext);
            ...
            try {
                ...
                result = cranked->invokeEmulatedFunction<int32_t, ArgType::int32_t, ArgType::uint32_t>(
                        cranked->emulatedLuaFunctions[index], 0);
            } catch (std::exception &ex) {
